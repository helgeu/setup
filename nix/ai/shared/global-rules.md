...
- **Search with the dedicated grep tool or `rg`, never `grep --include`.** RTK rewrites `grep`→`rtk grep`, which forwards flags straight to ripgrep — so grep-only flags break: `--include` errors with `rg: unrecognized flag --include`, and `grep -E` (extended regex) collides with rg's `-E` (encoding) → `unknown encoding`. Working forms: the agent's grep/glob tools (use their `include` param), or shell out with ripgrep syntax — `rg -g '*.ts' <pattern>` (quote the glob) or `rtk grep -t ts <pattern>` (file-type), and `rg --files -g '<glob>'` to list files. Never pass `--include`.
...
- **Write in Simplified Technical English (ASD-STE100).** Use short sentences, the active voice, one instruction per step, simple/consistent words, and present tense. Apply this to all communication and prose — chat replies, docs, PR and work-item descriptions, and code comments. Keep required formats correct: obey the Conventional Commits structure (`<type>(<scope>): <description>`) and all code syntax, but still write the descriptive text in STE. Do not let STE break a required format.
