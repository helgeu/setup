...
Read `~/.claude/ado-cli-reference.md` for ADO CLI flag rules and PR thread API structure.
...
Rules for posting:
- **Only anchor to files and lines that actually exist in this PR.** Before posting, validate every finding's file path and line range against the PR's iteration `changeEntries` (the authoritative changed-files list from the ADO reference). Never comment on a file the PR doesn't touch or a line outside the changed range — floating/dangling comments referencing non-existent files are a hard failure.
- **Comments go on the PR under review.** When asked to review PR `<id>`, post the findings back to that PR (or, in headless mode, emit the findings JSON the poster consumes) — do not just print them to the chat.
